...
class excitation_blue_heat_rabi(base_excitation):
    from resonator.scripts.PulseSequences.blue_heat_rabi import blue_heat_rabi
    name = 'ExcitationBlueHeatRabi'
    pulse_sequence = blue_heat_rabi

class excitation_ramsey(base_excitation):
    from resonator.scripts.PulseSequences.ramsey import ramsey
    name = 'ExcitationRamsey'
    pulse_sequence = ramsey
    
class excitation_ramsey_spectrum(base_excitation):
    from resonator.scripts.PulseSequences.pulsed_scan_ramsey import pulsed_scan_ramsey
    name = 'ExcitationRamsey'
    pulse_sequence = pulsed_scan_ramsey

class excitation_dephase(base_excitation):
    from resonator.scripts.PulseSequences.dephasing_chain import dephasing_chain
    name = 'DephasingChain'
    pulse_sequence = dephasing_chain
    
class excitation_ramsey_2ions(base_excitation):
    from resonator.scripts.PulseSequences.ramsey_2ions import ramsey_2ions
    name = 'Ramsey2ions'
