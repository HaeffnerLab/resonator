...
class voltage_conversion():
    ...
    def converter(voltage):
        ...
        if( 1.333 <= voltage <= 1.627):
            ZL = 1.292788078
            ZU = 1.628987089
            f1=calculation(coefficient1,ZU,ZL,voltage)
            return f1
    
        elif (1.119 <= voltage <= 1.333):
            ZL = 1.114338319
            ZU = 1.380313646
            f2=calculation(coefficient2,ZU,ZL,voltage)
            return f2
            ...
            ZL = 0.4881727745
            ZU = 1.029436865
            f4=calculation(coefficient4,ZU,ZL,voltage)
            return f4
        else:
            return 0

    def calculation(coefficient,ZU,ZL,Z):
        TempK=0
        count=0
        X = ((Z-ZL)-(ZU-Z))/(ZU-ZL)
